...
class MHRJoints:
    ...
    debug24: Indices confirmed by visual testing.
...
class SAM3DJoints:
    ...
    debug24: Fixed to match actual pred_keypoints_3d format.
    """
...
def detect_foot_contact(
    ...
    # debug24: Log foot contact debug info for frame 0
    if frame_idx == 0:
        print(f"[Motion Analyzer] debug24: ===== FOOT CONTACT DEBUG =====")
...
def create_motion_debug_overlay(
    ...
    # debug24: DYNAMIC INDEX MAPPING based on source
    ...
    print(f"[Motion Analyzer] debug24: Using {format_name} index mapping for overlay")
    print(f"[Motion Analyzer] debug24: HEAD={idx_map['HEAD']}, PELVIS={idx_map['PELVIS']}, L_ANKLE={idx_map['L_ANKLE']}, R_ANKLE={idx_map['R_ANKLE']}")
...
class SAM4DMotionAnalyzer:
    ...
    def analyze(
        ...
        # ===== PER-FRAME ANALYSIS =====
        # ===== JOINT INDICES =====
        # debug24: Use SMPLH indices since we PROJECT joint_coords to 2D
        ...
        # debug24: CONFIRMED pred_keypoints_2d (MHR) is correct!
        ...
        print(f"[{get_timestamp()}] [Motion Analyzer] debug24: 2D format = {format_2d}")
        ...
        for i in range(num_frames):
            ...
            # debug24: CONFIRMED pred_keypoints_2d is CORRECT!
            ...
            if has_kp_2d and i < len(keypoints_2d_list) and keypoints_2d_list[i] is not None:
                ...
                if i == 0:
                    print(f"[{get_timestamp()}] [Motion Analyzer] debug24: Using pred_keypoints_2d DIRECTLY (confirmed correct)")
                ...
                if i == 0:
                    print(f"[{get_timestamp()}] [Motion Analyzer] debug24: FALLBACK to joint_coords projection")
                    subject_motion["joints_2d_source"] = joints_2d_source
            else:
